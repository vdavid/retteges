...
export function getAllAudioNames(stages: Stage[]): string[] {
    return stages
        .map((stage: Stage) => {
            if (stage.type === 'start') {
                return [
                    (stage as StartStage).few,
                    (stage as StartStage).many,
...
export function getAudioNamesToPlay(stage: Stage, selectedCharacters: Character[]): string[] {
    return stage.type === 'start'
        ? [
              selectedCharacters.length <= 6
                  ? (stage as StartStage).few
                  : selectedCharacters.length < 18
                    ? (stage as StartStage).many
                    : (stage as StartStage).all,
          ]
        : stage.audioFiles || []
...
// Play audio snippets in sequence
// Returns true if the audio was canceled
export async function playAudio(
    snippets: AudioSnippet[],
    breakMs: number,
    abortSignal: AbortSignal,
): Promise<boolean> {
    // Cancel if there are no snippets
    if (!snippets.length) {
        return true
    }
    let canceled = false
    let abortEventListener:
